...
class HexGrid {
  ...
  public handleResize: EventListener;
  ...
  constructor(container: HTMLElement, size: number = 10) {
    ...
    for (let i = 0; i < shapesPerRow; i++) {
      const row = [];
      for (let j = 0; j < shapesPerColumn; j++) {
   ...
   * @memberof HexGrid
   */
  public getHexagon({ x, y }: Vector): Hexagon | false {
    try {
   ...
   * @memberof HexGrid
   */
  public getLine({ x, y }: Vector): HexGroup {
   ...
   * @memberof HexGrid
   */
  public getHoneyComb({ x, y }: Vector): HoneyComb {
    const honeyComb: HoneyComb = new HoneyComb(
      y % 2 === 0 ? this.getEvenYHoneyComb({ x, y }) : this.getOddYHoneyComb({ x, y })
    );
    return honeyComb;
  }
